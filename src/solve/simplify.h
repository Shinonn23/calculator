...
namespace math_solver {
    ...
    // Helper: GCD of two non-negative integers
    inline int64_t integer_gcd(int64_t a, int64_t b) {
        a = std::abs(a);
        b = std::abs(b);
        while (b != 0) {
            int64_t t = b;
    ...
    // Helper: LCM of two positive integers
    inline int64_t integer_lcm(int64_t a, int64_t b) {
        if (a == 0 || b == 0)
            return 1;
        return std::abs(a) / integer_gcd(a, b) * std::abs(b);
    }

    // Reduce an equation's LinearForm by dividing all terms by their GCD.
    // e.g., 4a + 8b - 16 = 0  becomes  a + 2b - 4 = 0  (displayed as a + 2b =
    // 4)
    inline void reduce_equation_by_gcd(LinearForm& form) {
        // Collect absolute values of all non-zero coefficients and constant
        std::vector<double> values;
        for (const auto& pair : form.coeffs) {
            if (std::abs(pair.second) > 1e-12) {
                values.push_back(std::abs(pair.second));
            }
        }
        if (std::abs(form.constant) > 1e-12) {
            values.push_back(std::abs(form.constant));
        }

        if (values.size() < 2)
            return; // Need at least 2 terms to reduce

        // Convert each value to a Fraction
        std::vector<Fraction> fracs;
        for (double v : values) {
            fracs.push_back(double_to_fraction(v));
        }

        // GCD of fractions: gcd(a/b, c/d) = gcd(a,c) / lcm(b,d)
        int64_t num_gcd = std::abs(fracs[0].numerator);
        int64_t den_lcm = fracs[0].denominator;

        for (size_t i = 1; i < fracs.size(); ++i) {
            num_gcd = integer_gcd(num_gcd, std::abs(fracs[i].numerator));
            den_lcm = integer_lcm(den_lcm, fracs[i].denominator);
        }

        if (num_gcd <= 0 || den_lcm <= 0)
            return;

        double divisor =
            static_cast<double>(num_gcd) / static_cast<double>(den_lcm);

        // Skip if divisor is effectively 1 or 0
        if (divisor < 1e-12 || std::abs(divisor - 1.0) < 1e-12)
            return;

        // Divide all terms by the GCD
        for (auto& pair : form.coeffs) {
            pair.second /= divisor;
        }
    ...
    class Simplifier {
        ...
        simplify(const Equation&        eq,
            ...
            // Reduce by GCD (e.g., 4a + 8b = 16 → a + 2b = 4)
            reduce_equation_by_gcd(normalized);
